...
void WeightedFmeasureLoss2Layer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void WeightedFmeasureLoss2Layer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void WeightedFmeasureLoss2Layer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void WeightedFmeasureLoss2Layer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
    const vector<Blob<Dtype>*>& bottom) {
  if (propagate_down[1] || propagate_down[2] || propagate_down[3]) {
    LOG(FATAL) << this->type()
               << " WeightedFmeasureLoss2Layer cannot backpropagate to label inputs, or weight maps.";
  }
  if (propagate_down[0]) {
...
#ifdef CPU_ONLY
STUB_GPU(WeightedFmeasureLoss2Layer);
#endif

INSTANTIATE_CLASS(WeightedFmeasureLoss2Layer);
REGISTER_LAYER_CLASS(WeightedFmeasureLoss2);
