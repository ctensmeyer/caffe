...
void Graph4CCutLayer<Dtype>::Forward_cpu(
  ...
  for (int n = 0; n < num; n++) {
    int num_offset = 4 * n * spatial_size;
    ...
    for (int h = 0; h < height; h++) {
      for (int w = 0; w < width; w++) {
		...
		//DLOG(INFO) << "\ts/t: " << source_edge << " " << term_edge;
  
        // pair weights.  Need to set each direction
		// edge weights are multiplied by 2 because the underlying library doesn't cut the edges
		// in each direction and the energy layer computes energy for each directed pair.
  	    if (h > 0) {
  	      // up neb
  	      spatial_offset = (h - 1) * height + w;
  	      Dtype neb_edge = 2 * edge_weights[num_offset + (UD_CHANNEL * spatial_size) + spatial_offset];
  	      grid->set_neighbor_cap(grid->node_id(h,w), -1, 0, neb_edge);
		  //DLOG(INFO) << "\tup: " << neb_edge; 
  	    }
  	    if (h + 1 < height) {
  	      // down neb
  	      ...
  	      Dtype neb_edge = 2 * edge_weights[num_offset + (UD_CHANNEL * spatial_size) + spatial_offset];
  	      grid->set_neighbor_cap(grid->node_id(h,w), +1, 0, neb_edge);
		  //DLOG(INFO) << "\tdown: " << neb_edge; 
  	    }
  	    if (w > 0) {
  	      // left neb
  	      spatial_offset = h * height + w - 1;
  	      Dtype neb_edge = 2 * edge_weights[num_offset + (LR_CHANNEL * spatial_size) + spatial_offset];
  	      grid->set_neighbor_cap(grid->node_id(h,w), 0, -1, neb_edge);
		  //DLOG(INFO) << "\tleft: " << neb_edge; 
  	    }
  	    if (w + 1 < width) {
  	      // right neb
  	      spatial_offset = h * height + w;
  	      Dtype neb_edge = 2 * edge_weights[num_offset + (LR_CHANNEL * spatial_size) + spatial_offset];
  	      grid->set_neighbor_cap(grid->node_id(h,w), 0, +1, neb_edge);
		  //DLOG(INFO) << "\tright: " << neb_edge; 
  	    }
  	  }
    }
    grid->compute_maxflow();
  
    int top_num_offset = n * spatial_size;
    for (int h = 0; h < height; h++) {
      for (int w = 0; w < width; w++) {
  	    int spatial_offset = h * height + w;
  	    segmentation[top_num_offset + spatial_offset] = (Dtype) grid->get_segment(grid->node_id(h,w));
		//DLOG(INFO) << "\th/w/s: " << h << " " << w << " " << (Dtype) grid->get_segment(grid->node_id(h,w));
  	  }
    }
	//DLOG(INFO) << "\tcost: " << grid->get_flow();
