...
LayerParameter HVPLayer<Dtype>::GetPoolingParam(const int partition_num,
      const bool is_horizontal, const int bottom_h, const int bottom_w, 
	  const HVPParameter hvp_param) {
  LayerParameter pooling_param;
  int num_bins = pow(2, partition_num);
  ...
  //LOG(INFO) << "Creating horz partition: " << is_horizontal << "\tlevel: " << 
 // 	partition_num << ", " << kernel_h << ", " << pad_h << ", " << kernel_w << ", " << pad_w;
  ...
  switch (hvp_param.pool()) {
  case HVPParameter_PoolMethod_MAX:
    pooling_param.mutable_pooling_param()->set_pool(
        PoolingParameter_PoolMethod_MAX);
    break;
  case HVPParameter_PoolMethod_AVE:
    pooling_param.mutable_pooling_param()->set_pool(
        PoolingParameter_PoolMethod_AVE);
    break;
  case HVPParameter_PoolMethod_STOCHASTIC:
    pooling_param.mutable_pooling_param()->set_pool(
...
template <typename Dtype>
void HVPLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  HVPParameter hvp_param = this->layer_param_.hvp_param();
  ...
  num_horz_partitions_ = hvp_param.num_horz_partitions();
  num_vert_partitions_ = hvp_param.num_vert_partitions();
  num_pool_regions_ = num_horz_partitions_ + num_vert_partitions_ + 1;
  ...
  for (int i = 0; i < num_pool_regions_; i++) {
  ...
  for (int i = 0; i < num_pool_regions_; i++) {
    ...
    // pooling layer setup
	LayerParameter pooling_param;
	if (i <= num_horz_partitions_) {
	  // horizontal partition
      pooling_param = GetPoolingParam(i, true, bottom_h_, bottom_w_, hvp_param);
	} else {
	  // vertical partition
      pooling_param = GetPoolingParam(i - num_horz_partitions_, false, bottom_h_, bottom_w_, hvp_param);
	}
...
template <typename Dtype>
void HVPLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void HVPLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void HVPLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  if (!propagate_down[0]) {
    return;
  }
  vector<bool> concat_propagate_down(num_pool_regions_, true);
...
INSTANTIATE_CLASS(HVPLayer);
REGISTER_LAYER_CLASS(HVP);
