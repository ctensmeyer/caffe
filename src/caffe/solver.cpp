...
Dtype SGDSolver<Dtype>::GetLearningRate() {
  ...
  if (lr_policy == "fixed") {
    ...
    if (this->steps_no_improvement_ >= this->param_.max_steps_without_improvement()) {
	  periods_no_improvement_++;
	  this->steps_no_improvement_ = 0;
	  if (periods_no_improvement_ >= this->param_.max_periods_without_improvement()) {
	    this->stop_early_ = true;
	  }
	}
    rate = this->param_.base_lr() *
        pow(this->param_.gamma(), periods_no_improvement_);
  } else {
...
void SGDSolver<Dtype>::ClipGradients() {
  ...
  const vector<shared_ptr<Blob<Dtype> > >& net_params = this->net_->params();
  if (this->param_.clip_gradients_global()) {
    Dtype sumsq_diff = 0;
    for (int i = 0; i < net_params.size(); ++i) {
    ...
    if (l2norm_diff > clip_gradients) {
      ...
      LOG(INFO) << "Gradient clipping: scaling down all gradients (L2 norm "
    ...
    for (int i = 0; i < net_params.size(); ++i) {
      if (this->net_->param_owners()[i] < 0) {
        Dtype sumsq_diff = net_params[i]->sumsq_diff();
        const Dtype l2norm_diff = std::sqrt(sumsq_diff);
        const Dtype scale_factor = clip_gradients / l2norm_diff;

	    const string& layer_name = this->net_->layer_names()[this->net_->param_layer_indices()[i].first];
        const string& param_display_name = this->net_->param_display_names()[i];
        if (l2norm_diff > clip_gradients) {
          LOG(INFO) << "Gradient clipping " << layer_name << ", param " << param_display_name
		      << " with gradient ("
              << l2norm_diff << " > " << clip_gradients << ") "
              << "by scale factor " << scale_factor;
		  net_params[i]->scale_diff(scale_factor);
