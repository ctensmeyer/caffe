...
class SigmoidCrossEntropyLoss2LayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  SigmoidCrossEntropyLoss2LayerTest()
  ...
  virtual ~SigmoidCrossEntropyLoss2LayerTest() {
  ...
  void TestForward() {
    ...
    for (int i = 0; i < 100; ++i) {
      ...
      SigmoidCrossEntropyLoss2Layer<Dtype> layer(layer_param);
...
TYPED_TEST_CASE(SigmoidCrossEntropyLoss2LayerTest, TestDtypesAndDevices);

TYPED_TEST(SigmoidCrossEntropyLoss2LayerTest, TestSigmoidCrossEntropyLoss) {
  this->TestForward();
}

TYPED_TEST(SigmoidCrossEntropyLoss2LayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  SigmoidCrossEntropyLoss2Layer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
