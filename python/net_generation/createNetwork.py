...
def createTransformParam(phase, seed=None, test_transforms = [10,50,100], deploy=False, **kwargs):
	...
	if (phase == caffe.TRAIN or deploy) and 'noise_std' in kwargs:
		...
		if deploy:
			for t in tt:
				transforms[t].extend(make_transforms.make_gaussnoise_transforms(noise[0], t))
...
def createNetwork(sources, size, val_sources=None,  num_output=1000, concat=False, spp=False, batch_size=32, deploy=False, seed=None, shift_channels=None, scale_channels=None, **tparams):
	n = caffe.NetSpec()	
	#data
...
def createExperiment(ds, tags, group, experiment, num_experiments=1, spp = False, shift=None, scale=None, **tparams):
	...
	for s in range(1,num_experiments+1):
		...
		#common parameters
		params = dict(sources=list(sources_tr), size=size[0], num_output=OUTPUT_SIZES[ds], concat=same, spp=spp, shift_channels=shift, scale_channels=scale, batch_size=BATCH_SIZE[ds], **tparams)
