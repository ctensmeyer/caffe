...
/**
 * @brief Does horizontal-vertical partition pooling on the input image
 *        by taking the max, average, etc. within regions
 *        so that the result vector of different sized
 *        images are of the same size.
 */
template <typename Dtype>
class HVPLayer : public Layer<Dtype> {
 public:
  explicit HVPLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "HVP"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int MinTopBlobs() const { return 1; }
  // MAX POOL layers can output an extra top blob for the mask;
  // others can only output the pooled inputs.
 ...
 protected:
  ...
  // calculates the kernel and stride dimensions for the pooling layer,
  // returns a correctly configured LayerParameter for a PoolingLayer
  virtual LayerParameter GetPoolingParam(const int partition_level,
      const bool is_horizontal, const int bottom_h, const int bottom_w, 
	  const HVPParameter hvp_param);

  int num_horz_partitions_;
  int num_vert_partitions_;
  int num_pool_regions_;
  int bottom_h_, bottom_w_;
  int channels_;
  int kernel_h_, kernel_w_;
  int pad_h_, pad_w_;
...
class BilinearInterpolationLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  int num_, channel_, width_in_, width_out_, height_in_, height_out_;
};

template<typename Dtype>
class CenterOfMassLayer : public Layer<Dtype> {
 public:
  explicit CenterOfMassLayer(const LayerParameter& param) 
  	: Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  	const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "CenterOfMass"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
...
template<typename Dtype>
class PowCenterOfMassLayer : public Layer<Dtype> {
 public:
  explicit PowCenterOfMassLayer(const LayerParameter& param) 
  	: Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  	const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "PowCenterOfMass"; }
  virtual inline int ExactNumBottomBlobs() const { return 1; }
  virtual inline int ExactNumTopBlobs() const { return 1; }
